...
int commandParser(char *cmd, int length) {
    ...
    for (int i = 0; i < commandCount; i++) {
        ...
        if (!getIsProgram(cmdList[i])) {
            if (commandCount > 1) {
                printf(
                    "No se pueden utilizar pipes con comandos internos de la "
                    "shell\n");
                return -1;
            } else {
        ...
        if (getIsFullscreen(cmdList[i])) {
            if (commandCount > 1) {
                printf(
                    "No se pueden utilizar pipes con programas en pantalla "
                    "completa\n");
                return -1;
            }
            if (runInBackground[i]) {
                printf(
                    "No se pueden correr en background programas en pantalla "
                    "completa\n");
                return -1;
            }
            fullScreen = TRUE;
    ...
    for (int i = 0; i < commandCount; i++) {
        if (i == 0) {
            if (commandCount > 1) {
                ...
                pidList[i] =
                    execv(cmdList[i], (process_t)getCommandFunction(cmdList[i]),
    ...
    for (int i = 0; i < commandCount; i++) {
        if (!runInBackground[i]) sys_waitPID(pidList[i]);
    }

    if (fullScreen) cmd_resetScreen();
...
int prog_priorityTestA(char **args) {
    for (int i = 0; i < 400000000; i++)
        if (i % 5000000 == 0) printf("A");

    return 0;
}

int prog_priorityTestB(char **args) {
    for (int i = 0; i < 400000000; i++)
        if (i % 5000000 == 0) printf("B");

    return 0;
}

void cmd_priorityTest(void **args) {
    if (args[0] == NULL || args[1] == NULL) {
        printf("Argumentos invalidos\n");
        return;
    }

    int nice1 = atoi(args[0]);
    if (nice1 < 0 || nice1 > 5) {
        printf("Valor de prioridad invalido\n");
        return;
    }
    int nice2 = atoi(args[1]);
    if (nice2 < 0 || nice2 > 5) {
        printf("Valor de prioridad invalido\n");
        return;
    }
    ...
    printf(
        "El programa que imprime A tiene prioridad %d y el que imprime B tiene "
        "prioridad %d. Se recomienda elegir 0 y 5, por ejemplo, para apreciar "
        "mejor la diferencia de tiempo de procesamiento.\n",
        nice1, nice2);

    pid[0] =
        execv("Priority Test (Slave 1)", prog_priorityTestA, NULL, TRUE, NULL);

    pid[1] =
        execv("Priority Test (Slave 2)", prog_priorityTestB, NULL, TRUE, NULL);

    sys_setNiceness(pid[0], nice1);
    sys_setNiceness(pid[1], nice2);

    for (int i = 0; i < 2; i++) sys_waitPID(pid[i]);
}

void cmd_setNiceness(void **args) {
    if (args[0] == NULL || args[1] == NULL) {
        printf("Argumentos invalidos\n");
        return;
    }
...
int main() {
    ...
    command_register("cat", (function)prog_cat,
                     "Imprime el archivo especificado", TRUE, FALSE);
    ...
    command_register("testforeground", (function)testForeground,
                     ...
                     FALSE);
    command_register("echo", (function)prog_echo, "Echo como programa", TRUE,
                     FALSE);
    command_register("prueba", (function)prog_prueba, "Para correr con echo",
                     TRUE, FALSE);
    command_register("nice", cmd_setNiceness,
                     "Recibe un PID y un entero y establece la prioridad de "
                     "ese proceso. Prioridades entre 0 y 5",
                     FALSE, FALSE);
    command_register(
        "priorityTest", cmd_priorityTest,
        "Crea dos procesos, con prioridades N1 y N2 a ingresarse como "
        "parametro, uno imprime A y el otro B",
        FALSE, FALSE);
