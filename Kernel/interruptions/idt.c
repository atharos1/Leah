...
unsigned char inportb(unsigned short portid) { return _inportb(portid); }
...
#define PIT_CHANNEL1 \
    0x41  // PIT Channels 1's Data Register Port, we wont be using this here
...
void writeIDT() {
    ...
    outportb(0x70, 0x8B);		// set the index again (a read will
    reset the index to register D) outportb(0x71, prev | 0x40);	// write the
    previous value ORed with 0x40. This turns on bit 6 of register B*/
...
void pruebaRTC() {
    ...
    outportb(0x70, 0x8B);       // select register B, and disable NMI
    char prev = inportb(0x71);  // read the current value of register B
    outportb(0x70, 0x8B);  // set the index again (a read will reset the index
                           // to register D)
    outportb(0x71, prev | 0x40);  // write the previous value ORed with 0x40.
...
void dumpData(char *msg, uint64_t *RIP, uint64_t *RSP) {
    ...
    printf(
        "R8: %X, R9: %X, R10: %X, R11: %X, R12: %X, R13: %X, R14: %X, R15: "
        "%X\n",
...
void exDispatcher(int n, uint64_t *RIP, uint64_t *RSP, uint64_t r) {
    ...
    switch (n) {
        case 0:  // Division by 0
            ...
            dumpData("EXCEPCION (DIVISION POR CERO)", (uint64_t *)RIP_Back,
                     (uint64_t *)RSP_Back);
            printf("\n\n");
            break;

        case 6:  // InvalidOpCode
            kernelPanic("EXCEPCION (CODIGO DE OPERACION INVALIDO)", RIP,
                        RSP);  // Lo dejo porque la pantallita quedaba divertida
            //*RIP = (uint64_t)instructionPointerBackup;
            //*RSP = (uint64_t)stackPointerBackup;
            dumpData("EXCEPCION (CODIGO DE OPERACION INVALIDO)",
                     (uint64_t *)RIP_Back, (uint64_t *)RSP_Back);
            printf("\n\n");
            break;
    }
