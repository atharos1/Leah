...
void writeStringToScreen(int x, int y, char* string) {
	...
	for(int i = 0; (*aux_s) != '\0'; i++) {
		drawCharToScreen(aux_x, y, (*aux_s));
		aux_s++;
		aux_x += 8;
	}
...
void drawCharToScreen(int x, int y, char character) {
	...
	for(int i = 0; i < 8; i++) {
		for(int j = 0; j < 8; j++) {
			...
			if(bitIsPresent)
				drawPixel(aux_x, aux_y, 0xFFFFFF);
			else
				drawPixel(aux_x, aux_y, 0x0);
			aux_x++;
		}
		aux_x = x;
		aux_y++;
...
int main()
{
	...
	writeStringToScreen(x, y, string);
	y += 8;
