...
static timerFunction timerFunctions[MAX_FUNCTIONS] = {
    {0}};  // CONIRMAR SI GARANTIZA QUE TODOS LOS VALORES DE F VAN A EMPEZAR EN
           // 0

void timer_Restart() {
    for (int i = 0; i < MAX_FUNCTIONS; i++) timerFunctions[i].f = 0;
}
    ...
    for (i = 0; i < MAX_FUNCTIONS; i++) {
        if (timerFunctions[i].f == f) {
            ...
            for (j = i + 1; j < MAX_FUNCTIONS; j++) {
                ...
                timerFunctions[j - 1].remainingTicks =
                    timerFunctions[j].remainingTicks;
