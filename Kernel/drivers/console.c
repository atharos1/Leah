...
void setGraphicCursorStatus(unsigned int status) {
    if (cursorEnabled == status) return;

    if (status == 1) timer_appendFunction(cursorTick, PIT_FREQUENCY);

    if (status == 0) timer_removeFunction(cursorTick);
...
void cursorTick() {
    if (cursorStatus == 0)
        drawRectangle(curScreenCol * char_Width, curScreenRow * char_Height,
                      char_Width, char_Height, fontColor);
    else
        drawRectangle(curScreenCol * char_Width, curScreenRow * char_Height,
                      char_Width, char_Height, backgroundColor);
...
void shiftCursor(int cant) {
    ...
    if (newPos >= 0) setCursor(newPos % num_Cols, newPos / num_Cols);
}
...
void printChar(char c) {
    ...
    switch (c) {
        ...
        case BACKSPACE:
            resetCursor();
            shiftCursor(-1);
            drawChar(curScreenCol * char_Width, curScreenRow * char_Height, ' ',
        ...
        case TAB:
            ...
            for (int i = 0; i < cantSpaces; i++) printChar(' ');
            break;
        default:
            if (c < ' ' || c >= 0x80) break;
            drawChar(curScreenCol * char_Width, curScreenRow * char_Height, c,
                     fontSize, fontColor, backgroundColor);
            shiftCursor(1);
            break;
    }
    ...
    while (*format != '\0') {
        ...
        switch (*format) {
            case 'd':
                ...
                break;
            case 'c':
                printChar(va_arg(pa, int));  // NOTA: en parámatros ilimitados,
