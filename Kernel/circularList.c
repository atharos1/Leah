...
NODE *deleteByValue(NODE *tail, void *data, comparator cmp, int *cantDeleted,
                    int limit) {
    ...
    *cantDeleted = 0;
    if (tail == NULL)
        ...
        if (cmp(tail->data, data) == 0) {
            ...
            free(current);
            (*cantDeleted)++;
            if (*cantDeleted == limit) return tail;
        }
        // return tail;
    }
    do {
...
NODE *deleteHead(NODE *tail) { return deleteByLocation(tail, 1); }
