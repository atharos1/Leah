...
void sem_wait(sem_t sem) {
    ...
    if (sem->value == 0) {
        ...
        // printf("Agregando proceso %s de la cola del semáforo   ",
        // getProcessByPID(thread->process)->name);
...
void sem_signal(sem_t sem) {
    ...
    while (sem->lockedQueue != NULL) {
        ...
        if (t->status == DEAD)
            ...
            // printf("Sacando proceso %s de la cola del semáforo   ",
            // getProcessByPID(t->process)->name);
