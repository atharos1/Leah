...
void purgeThreadList(process_t *process, int close,
                     int called_from_kill_process);
...
void purgeProcessList(int close) {
    for (int i = 0; i < MAX_PROCESS_COUNT; i++) {
        if (processList[i] != NULL) {
            if (close == TRUE) killProcess(i, -1);
            processList[i] = NULL;
        }
    }
}

int processCount() { return process_count; }

int aliveProcessCount() { return alive_process_count; }

void erasePCB(process_t *process) {
    freeMemory(process->name);
    if (process->heap.size != 0) freeMemory(process->heap.base);
    freeMemory(process);
}
...
int getHeapSize(int pid) { return processList[pid]->heap.size; }

void *getHeapBase(int pid) { return processList[pid]->heap.base; }
...
void killProcess(int pid, int retValue) {
    if (pid == 0 || !isValidProcess(pid)) return;
    ...
    sem_signal(
        processList[pid]->finishedSem);  // Despertamos a potencial waitpid

    if (isCurrProcess) _force_scheduler();
}
...
int threadCount(int pid) {
    ...
    for (int i = 0; i < MAX_THREAD_COUNT; i++)
        if (processList[pid]->threadList[i] != NULL) cant++;
...
int getFreeTID(process_t *process) {
    for (int i = 0; i < MAX_THREAD_COUNT; i++)
        if (process->threadList[i] == NULL) return i;
...
void killThread(int pid, int tid, int called_from_kill_process) {
    ...
    if (t == NULL || t->status == DEAD) return;
    ...
    if (t->is_someone_joining == FALSE ||
        called_from_kill_process) {  // Solo borro si nadie me espera
    ...
    if (tid == 0 && !called_from_kill_process) killProcess(pid, -1);

    if (isCurrThread && !called_from_kill_process) _force_scheduler();
}
...
thread_t *createThread(process_t *process, void *code, void *args,
    ...
    if (isMain == TRUE)
        thread->stack.current = _initialize_stack_frame(
            &processWrapper, code,
            (char *)thread->stack.base + stack_size * PAGE_SIZE, args);
    else
        thread->stack.current = _initialize_stack_frame(
            &threadWrapper, code,
            (char *)thread->stack.base + stack_size * PAGE_SIZE, args);
...
void purgeFdList(process_t *process, int close) {
    for (int i = 0; i < MAX_FD_COUNT; i++) {
        if (process->fd_table[i] != NULL) {
            if (close == TRUE) closeFile(process->fd_table[i]);
...
void listProcess(ps_struct buffer[], int *bufferCount) {
    ...
    for (int i = 0; i < MAX_PROCESS_COUNT && n <= process_count; i++) {
        if (processList[i] != NULL) {
            ...
            buffer[i].niceness = processList[i]->nice;
            n++;
        }
    }
    *bufferCount = n;
...
process_t *getProcessByPID(int pid) { return processList[pid]; }
...
int getFreeFD(int pid) {
    for (int i = 3; i < MAX_FD_COUNT; i++)
        if (processList[pid]->fd_table[i] == NULL) return i;
